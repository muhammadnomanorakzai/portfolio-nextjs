...
export default function SectionHeading({ title, subtitle, centered = true }) {
  return (
    <motion.div
      ...
      className={`mb-5 pt-10 ${centered ? "text-center" : ""}`}>
      {/* Badge */}
      {/* <motion.div
      ...
      {subtitle && (
        <motion.p
          ...
          className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto leading-relaxed mt-2">
          {subtitle}
        </motion.p>
      )}
      ...
      <motion.div
        ...
        className={`h-1 bg-gradient-to-r from-primary-600 to-pink-600 rounded-full mt-8 ${centered ? "mx-auto" : ""}`}
      />
    </motion.div>
  );
}
